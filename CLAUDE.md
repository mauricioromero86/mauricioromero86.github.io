...
python tools/gen-research-jsonld.py   # regenerate _includes/research-jsonld.html
                                      # (ScholarlyArticle JSON-LD; MUST re-run after
                                      #  ANY research.qmd edit or the graph goes stale)
```
...
- **SEO/AI**: implement Google's SEO + AI optimization guides (per-page title/description,
  ...
  `.md` files. (Standard schema.org JSON-LD is *not* AI-specific markup: `WebSite`,
  `Person`/`ProfilePage`, and the `ScholarlyArticle` graph in
  `_includes/research-jsonld.html` — generated by `tools/gen-research-jsonld.py`, never
  hand-edited — are part of the SEO spec.)
